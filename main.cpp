...
bool init(){
    ...
    if(SDL_Init(SDL_INIT_VIDEO)<0){
        ...
        window=SDL_CreateWindow("Sorting visualzer",SDL_WINDOWPOS_UNDEFINED,SDL_WINDOWPOS_UNDEFINED,SCREEN_WIDTH,SCREEN_HEIGHT,SDL_WINDOW_SHOWN);
        if(window==NULL)
        {
...
int partition_array(int a[], int si, int ei) {
    int pivot = a[si]; // Choosing the first element as pivot
    int i = si+1;
    int j = ei;

    while (i < j) {
        while (i <= ei && a[i] <= pivot) {
            i++;
        }
        while (j >= si && a[j] > pivot) {
            j--;
        }
        if (i < j) {
            // Swap a[i] and a[j]
...
void merge2SortedArrays(int a[], int si, int ei,int mid)
{
...
void mergeSort(int a[], int si, int ei)
{
    ...
    merge2SortedArrays(a, si, ei,mid);
}
void insertionSort() {
